...
static void dump_log_init(VI_PIPE ViPipe, CVI_U16 frmNum)
{
	...
	CVI_ISP_AEMallocRawReplayExpBuf(ViPipe, frmNum + 10);
}
...
static CVI_S32 calculate_blk_cnt(CVI_U32 u32BlkSize)
{
	...
	total_mem = get_ion_size_info("/sys/kernel/debug/ion/cvi_vpp_heap_dump/total_mem");
	alloc_mem = get_ion_size_info("/sys/kernel/debug/ion/cvi_vpp_heap_dump/alloc_mem");
#else
...
static void *dump_smooth_raw_thread(void *param)
{
	...
	CVI_U16 TotalFrameCnt = pstRawDumpInfo->u32TotalFrameCnt;
...
static CVI_S32 dump_smooth_raw(VI_PIPE ViPipe, const RAW_DUMP_INFO_S *pstRawDumpInfo)
{
	...
	stPubAttr.f32FrameRate = stPubAttr.f32FrameRate > 0 ? stPubAttr.f32FrameRate : 10;
	//wait for ae response frames
	usleep(10 * 1000 / stPubAttr.f32FrameRate * 1000);
	...
	return CVI_FAILURE;
#elif defined(__CV181X__) || defined(__CV180X__) || defined(__CV186X__) // TODO: A2 has tile mode
#else
#error "should check here when porting new chip"
	...
	if (TotalFrameCnt > VB_POOL_MAX_BLK) {
		poolCnt = (TotalFrameCnt + VB_POOL_MAX_BLK - 1) / VB_POOL_MAX_BLK;
		vbFifoPoolID = (VB_POOL *)calloc(1, sizeof(VB_POOL) * poolCnt);
		memset(vbFifoPoolID, VB_INVALID_POOLID, poolCnt);
		for (int i = 0; i < poolCnt; i++) {
			if (i == (poolCnt-1)) {
				cfg.u32BlkCnt = (TotalFrameCnt % VB_POOL_MAX_BLK == 0)
						 ? VB_POOL_MAX_BLK : (TotalFrameCnt % VB_POOL_MAX_BLK);
				vbFifoPoolID[i] = CVI_VB_CreatePool(&cfg);
			} else {
				cfg.u32BlkCnt = VB_POOL_MAX_BLK;
				vbFifoPoolID[i] = CVI_VB_CreatePool(&cfg);
			}
			RETURN_FAILURE_IF(vbFifoPoolID[i] == VB_INVALID_POOLID);
			LOGOUT("create vb fifo pool: %d, blkCnt: %d, blkSize: %d\n", vbFifoPoolID[i], cfg.u32BlkCnt, cfg.u32BlkSize);
		}
		FIFO_INIT(&vb_fifo, TotalFrameCnt);
	} else {
		vbFifoPoolID = (VB_POOL *)calloc(1, sizeof(VB_POOL));
		vbFifoPoolID[0] = CVI_VB_CreatePool(&cfg);
		RETURN_FAILURE_IF(vbFifoPoolID[0] == VB_INVALID_POOLID);
		FIFO_INIT(&vb_fifo, cfg.u32BlkCnt);
		LOGOUT("create vb fifo pool: %d, blkCnt: %d, blkSize: %d\n", vbFifoPoolID[0], cfg.u32BlkCnt, cfg.u32BlkSize);
	}
	...
	CVI_U32 WidthBytes = vbFifoBlkSize / RoiRect.u32Height / frm_num;
	CVI_U32 strideSrc = fullVbBlkSize / stChnAttr.stSize.u32Height;
	CVI_U64 addrOffsetSrc = strideSrc * RoiRect.s32Y;

#if USE_CDMA
	CVI_CDMA_2D_S cdmaParam;
	cdmaParam.u16Height = RoiRect.u32Height;
	cdmaParam.u16StrideSrc = strideSrc;
	cdmaParam.u16Width = WidthBytes;
	cdmaParam.u16StrideDst = cdmaParam.u16Width;
#else
	CVI_TDMA_2D_S tdmaParam;
	tdmaParam.h = RoiRect.u32Height;
	tdmaParam.stride_bytes_src = strideSrc;
	tdmaParam.w_bytes = WidthBytes;
	...
	LOGOUT("full size w: %d, h: %d, roi size w: %d, h: %d,\
		...
		strideSrc, WidthBytes,
		(CVI_U32) addrOffsetSrc);
	...
	for (int i = 0; i < TotalFrameCnt; i++) {
		...
		vbElm.vb = CVI_VB_GetBlock(vbFifoPoolID[i/VB_POOL_MAX_BLK], vbFifoBlkSize);
...
#if USE_CDMA
				cdmaParam.u64PhyAddrDst = paddr_dst;
				cdmaParam.u64PhyAddrSrc = stVideoFrameFull[j].stVFrame.u64PhyAddr[0] + addrOffsetSrc;
#else
				tdmaParam.paddr_dst = paddr_dst;
				tdmaParam.paddr_src = stVideoFrameFull[j].stVFrame.u64PhyAddr[0] + addrOffsetSrc;
#endif
...
#define MEASURE_TDMA_COPY_TIME 0
	...
	while (rawFidEnd <= rawFid + TotalFrameCnt) {
		CVI_ISP_GetFrameID(ViPipe, &rawFidEnd);
	}
	dump_log(ViPipe, name);
	...
	for (int i = 0; i < poolCnt; i++) {
		s32Ret = CVI_VB_DestroyPool(vbFifoPoolID[i]);
		ERROR_IF(s32Ret != CVI_SUCCESS);
	}
	if (vbFifoPoolID != NULL) {
		free(vbFifoPoolID);
	}
...
fail:
	...
	for (int i = 0; i < poolCnt; i++) {
		if (vbFifoPoolID[i] != VB_INVALID_POOLID) {
			s32Ret = CVI_VB_DestroyPool(vbFifoPoolID[i]);
		}
	}
	if (vbFifoPoolID != NULL) {

							...
							CVI_U32 totalFrame, CVI_U32 curFrame, CVI_U32 rawFrameSize)
{
	...
	if (curFrame == 0) {
		...
		if (access("/mnt/data/rawTestFlag", F_OK) == 0) {
			...
			#define TEST_RAW_PREFIX "/mnt/data/rawTest"

			char path[_BUFF_LEN] = {0};
			char cmd[_BUFF_LEN] = {0};

			snprintf(path, _BUFF_LEN, "%s/%s", TEST_RAW_PREFIX,
			...
			snprintf(path, _BUFF_LEN, "%s/%s/header.bin", TEST_RAW_PREFIX,
				pRawReplayCtx->pRawHeader[0].enWDR ? "wdr" : "sdr");
			...
			snprintf(path, _BUFF_LEN, "%s/%s/test.raw", TEST_RAW_PREFIX,
				pRawReplayCtx->pRawHeader[0].enWDR ? "wdr" : "sdr");

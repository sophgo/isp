							...
							CVI_U32 totalFrame, CVI_U32 curFrame, CVI_U32 rawFrameSize)
{
	...
	if (curFrame == 0) {
		...
		if (pRawReplayCtx->u32RoiTotalFrame != 0 &&
...
#if USE_CDMA
			pRawReplayCtx->stCDMAParam[0].u16StrideSrc = strideBytesSrc;
			pRawReplayCtx->stCDMAParam[0].u16StrideDst = strideBytesDst;
			pRawReplayCtx->stCDMAParam[0].u16Height = heightSrc;
			pRawReplayCtx->stCDMAParam[0].u16Width = strideBytesSrc;
			pRawReplayCtx->stCDMAParam[0].u64PhyAddrSrc = pRawReplayCtx->u64RoiPhyAddr[0];
			pRawReplayCtx->stCDMAParam[0].u64PhyAddrDst = pRawReplayCtx->u64PhyAddr +
				strideBytesDst * pRawInfo->stRoiRect.s32Y + pRawInfo->stRoiRect.s32X / 2 * 3;
#else
			pRawReplayCtx->stTDMAParam[0].stride_bytes_src = strideBytesSrc;
			pRawReplayCtx->stTDMAParam[0].stride_bytes_dst = strideBytesDst;
			pRawReplayCtx->stTDMAParam[0].h = heightSrc;
			pRawReplayCtx->stTDMAParam[0].w_bytes = strideBytesSrc;
			...
			if (pRawInfo->enWDR) {
#if USE_CDMA
				pRawReplayCtx->stCDMAParam[1].u16StrideSrc = strideBytesSrc;
				pRawReplayCtx->stCDMAParam[1].u16StrideDst = strideBytesDst;
				pRawReplayCtx->stCDMAParam[1].u16Height = heightSrc;
				pRawReplayCtx->stCDMAParam[1].u16Width = strideBytesSrc;
				pRawReplayCtx->stCDMAParam[1].u64PhyAddrSrc = pRawReplayCtx->u64RoiPhyAddr[1];
				pRawReplayCtx->stCDMAParam[1].u64PhyAddrDst = pRawReplayCtx->u64PhyAddr +
					pRawReplayCtx->u32BlkSize / 2 + strideBytesDst * pRawInfo->stRoiRect.s32Y
					+ pRawInfo->stRoiRect.s32X / 2 * 3;
#else
				pRawReplayCtx->stTDMAParam[1].stride_bytes_src = strideBytesSrc;
				pRawReplayCtx->stTDMAParam[1].stride_bytes_dst = strideBytesDst;
				pRawReplayCtx->stTDMAParam[1].h = heightSrc;
				pRawReplayCtx->stTDMAParam[1].w_bytes = strideBytesSrc;
...
#define START_SENSOR 1
	...
	stPipeAttr.u32MaxW = VI_DEV_MAX_WIDTH;
	stPipeAttr.u32MaxH = VI_DEV_MAX_HEIGHT;
...
#if USE_CDMA
			pRawReplayCtx->stCDMAParam[0].u64PhyAddrSrc = tempPhyAddrLe;
			s32Ret = CVI_SYS_CDMACopy2D(&pRawReplayCtx->stCDMAParam[0]);
#else
...
#if USE_CDMA
			pRawReplayCtx->stCDMAParam[1].u64PhyAddrSrc = tempPhyAddrSe;
			s32Ret = CVI_SYS_CDMACopy2D(&pRawReplayCtx->stCDMAParam[1]);
#else
...
static void *raw_replay_thread(void *param)
{
	...
	ERROR_IF(s32Ret != CVI_SUCCESS);
#endif
	while ((stWBInfo.u16ColorTemp == 0) || (stWBInfo.u16Grgain == 0)) {//waiting for awb run
		s32Ret = CVI_ISP_QueryWBInfo(pRawReplayCtx->ViPipe, &stWBInfo);
		ERROR_IF(s32Ret != CVI_SUCCESS);
	}
	while (pRawReplayCtx->bRawReplayThreadEnabled) {

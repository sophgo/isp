...
CVI_U32 CVI_BIN_GetSingleISPBinLenWrap(enum CVI_BIN_SECTION_ID id)
{
	...
	if (isp_mgr_buf_is_client(ViPipe)) {
		...
		memset(&msg, 0, sizeof(ISP_IPC_MSG_S));
		msg.cmd = ISP_IPC_BIN_GET_SINGLE_LEN;
		msg.arg_len = sizeof(enum CVI_BIN_SECTION_ID);
		msg.res_len = sizeof(CVI_U32);

		client_send_cmd_to_server(&msg, (CVI_U8 *) &id, (CVI_U8 *) &binSize);

	} else {
		binSize = CVI_BIN_GetSingleISPBinLen(id);
	}
...
#define DESC_SIZE 624 // from cvi_bin.c !!!!!!!
	...
	if (isp_mgr_buf_is_client(ViPipe)) {
		...
		memset(&msg, 0, sizeof(ISP_IPC_MSG_S));
		msg.cmd = ISP_IPC_BIN_EXPORT_SINGLE_BIN;
		msg.arg_len = sizeof(enum CVI_BIN_SECTION_ID);
		msg.res_len = u32DataLength;

		s32Ret = client_send_cmd_to_server(&msg, (CVI_U8 *) &id, pu8Buffer);
		...
		s32Ret = CVI_BIN_ExportSingleISPBinData(id, pu8Buffer, u32DataLength);
	}
...
CVI_S32 CVI_BIN_LoadParamFromBinExWrap(enum CVI_BIN_SECTION_ID id, CVI_U8 *pu8Buffer, CVI_U32 u32DataLength)
{
	...
	if (isp_mgr_buf_is_client(ViPipe)) {
		...
		memset(&msg, 0, sizeof(ISP_IPC_MSG_S));
		msg.cmd = ISP_IPC_BIN_IMPORT_SINGLE_BIN;
		msg.arg_len = sizeof(enum CVI_BIN_SECTION_ID);
		msg.res_len = u32DataLength;
		...
		s32Ret = CVI_BIN_LoadParamFromBinEx(id, pu8Buffer, u32DataLength);
	}

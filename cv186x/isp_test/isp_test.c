...
CVI_S32 CVI_ISP_RandomInitDPDynamicAttr(ISP_DP_DYNAMIC_ATTR_S *pstDPCDynamicAttr)
{
	...
	pstDPCDynamicAttr->stManual.BrightDefectToNormalPixRatio = rand() % 1024;
	pstDPCDynamicAttr->stManual.DarkDefectToNormalPixRatio = rand() % 1024;
	...
	for (i = 0; i < ISP_AUTO_ISO_STRENGTH_NUM; i++) {
		...
		pstDPCDynamicAttr->stAuto.BrightDefectToNormalPixRatio[i] = rand() % 1024;
		pstDPCDynamicAttr->stAuto.DarkDefectToNormalPixRatio[i] = rand() % 1024;
...
#if 0
#define RAW_REPLAY_TEST_RES_PATH_CV183X  "/mnt/data/rawTest_cv183x"
#define RAW_REPLAY_TEST_RES_PATH_cv182x  "/mnt/data/rawTest_cv182x"

static void isp_raw_replay_test_init(void)
{
...
CVI_S32 isp_raw_replay_test(CVI_VOID)
{
	...
	s32Ret = CVI_SYS_GetChipId(&chip_id);

	if (IS_CHIP_CV183X(chip_id)) {
		prefix = RAW_REPLAY_TEST_RES_PATH_CV183X;
	} else if (IS_CHIP_CV182X(chip_id)) {
		prefix = RAW_REPLAY_TEST_RES_PATH_cv182x;
	} else {
		printf("error: unknown chip......\n");
		return CVI_FAILURE;
	}

	snprintf(path, 128, "%s/%s", prefix, "sdr");

	if (access(path, F_OK) == 0) {

		isp_raw_replay_test_init();

		sleep(1);

		s32Ret = start_raw_replay_auto_test(0, path);
		...
		printf("warning: sdr raw replay test res not exist, skip......\n");

	}

	snprintf(path, 128, "%s/%s", prefix, "wdr");
	...
	if (access(path, F_OK) == 0) {
		...
		printf("warning: wdr raw replay test res not exist, skip......\n");

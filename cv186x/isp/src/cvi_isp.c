...
#ifdef ENABLE_ISP_IPC
	isp_ipc_server_cmd_init();
	ret = isp_ipc_server_init();
#endif
...
#ifdef ENABLE_ISP_IPC
static CVI_S32 __get_server_buf(VI_PIPE ViPipe, CVI_U64 *pu64PhyAddr)
{
	ISP_IPC_MSG_S msg;
	CVI_S32 ret = CVI_SUCCESS;
	...
	msg.cmd = ISP_IPC_INIT;
	msg.arg_len = sizeof(VI_PIPE);
	msg.res_len = sizeof(CVI_U64);

	ret = client_send_cmd_to_server(&msg, (CVI_U8 *) &ViPipe, (CVI_U8 *) &u64PhyAddr);
...
CVI_S32 CVI_ISP_Client_Init(VI_PIPE ViPipe)
{
	...
	__get_server_buf(ViPipe, &u64PhyAddr);

	isp_mgr_buf_init(ViPipe, u64PhyAddr);
...
CVI_S32 CVI_ISP_Run(VI_PIPE ViPipe)
{
	...
	while (g_isp_thread_run) {
		...
		if (FD_ISSET(fd, &efds)) {
			...
			memset(&ev, 0, sizeof(ev));
			ec.id = VI_IOCTL_DQEVENT;
			ec.ptr = (void *)&ev;
			ecs.count = 1;
			ecs.controls = &ec;

			if (ioctl(fd, VIDIOC_G_EXT_CTRLS, &ecs) < 0) {
				fprintf(stderr, "VIDIOC_G_EXT_CTRLS - %s NG\n", __func__);
				break;
			} else {
				ISP_LOG_DEBUG("event type: %d\n", ev.type);
			}
#endif
			switch (ev.type) {
			...
			{
				tempViPipe = ev.type - VI_EVENT_PRE0_SOF;
#ifdef V4L2_ISP_ENABLE
				if (g_astIspCtx[tempViPipe] == NULL || g_astIspCtx[tempViPipe]->ispDevFd <= 0) {
					continue;
				}
#endif
				ISP_LOG_INFO("pipe %d sof+++,%d\n", tempViPipe, ev.frame_sequence);
				isp_mgr_pre_sof(tempViPipe, ev.frame_sequence);
				ISP_LOG_INFO("pipe %d sof---\n", tempViPipe);
				break;
			}
			case VI_EVENT_PRE0_EOF:
			case VI_EVENT_PRE1_EOF:
			case VI_EVENT_PRE2_EOF:
			case VI_EVENT_PRE3_EOF:
			case VI_EVENT_PRE4_EOF:
			case VI_EVENT_PRE5_EOF:
			{
				tempViPipe = ev.type - VI_EVENT_PRE0_EOF;
#ifdef V4L2_ISP_ENABLE
				if (g_astIspCtx[tempViPipe] == NULL || g_astIspCtx[tempViPipe]->ispDevFd <= 0) {
					continue;
				}
#endif
				ISP_LOG_INFO("pipe %d be eof+++,%d\n", tempViPipe, ev.frame_sequence);
				isp_mgr_pre_eof(tempViPipe, ev.frame_sequence);
				ISP_LOG_INFO("pipe %d be eof---\n", tempViPipe);
				break;
			}
			...
			{
				tempViPipe = ev.type - VI_EVENT_POST0_EOF;
#ifdef V4L2_ISP_ENABLE
				if (g_astIspCtx[tempViPipe] == NULL || g_astIspCtx[tempViPipe]->ispDevFd <= 0) {
					continue;
				}
#endif
				ISP_LOG_INFO("pipe %d post eof+++,%d\n", tempViPipe, ev.frame_sequence);
				isp_mgr_post_eof(tempViPipe, ev.frame_sequence);
				ISP_LOG_INFO("pipe %d post eof---\n", tempViPipe);
				break;
			}
			default:
				ISP_LOG_ERR("unknown vi event, %d\n", ev.type);
				break;
			}
		}
...
CVI_S32 CVI_ISP_Client_Exit(VI_PIPE ViPipe)
{
	...
	ret = isp_mgr_buf_uninit(ViPipe);
	isp_ipc_client_deinit();
#else
	UNUSED(ViPipe);
	ISP_LOG_ASSERT(0);
#endif
...
CVI_S32 CVI_ISP_SetClutHslAttr(VI_PIPE ViPipe, const ISP_CLUT_HSL_ATTR_S *pstClutSaturationAttr)
{
	ISP_LOG_DEBUG("+\n");
	if ((ViPipe < 0) || (ViPipe >= VI_MAX_PIPE_NUM)) {
		ISP_LOG_ERR("ViPipe %d value error\n", ViPipe);
	...
	ret = isp_clut_ctrl_set_clut_hsl_attr(ViPipe, pstClutSaturationAttr);

	return ret;
}

CVI_S32 CVI_ISP_GetClutHslAttr(VI_PIPE ViPipe, ISP_CLUT_HSL_ATTR_S *pstClutSaturationAttr)
{
	if ((ViPipe < 0) || (ViPipe >= VI_MAX_PIPE_NUM)) {
		ISP_LOG_ERR("ViPipe %d value error\n", ViPipe);
		return -ENODEV;
	...
	if (pTemp != NULL) {
		memcpy(pstClutSaturationAttr, pTemp, sizeof(ISP_CLUT_HSL_ATTR_S));
	}
	return ret;
}
...
CVI_S32 CVI_ISP_SetTNRMATFAttr(VI_PIPE ViPipe, const ISP_TNR_MATF_ATTR_S *pstTNRMATFAttr)
{
	...
	if (pstTNRMATFAttr == CVI_NULL) {
		return CVI_FAILURE;
	}

	CVI_S32 ret = CVI_SUCCESS;

	ret = isp_tnr_ctrl_set_tnr_matf_attr(ViPipe, pstTNRMATFAttr);
...
CVI_S32 CVI_ISP_GetTNRMATFAttr(VI_PIPE ViPipe, ISP_TNR_MATF_ATTR_S *pstTNRMATFAttr)
{
	...
	CVI_S32 ret = CVI_SUCCESS;
	const ISP_TNR_MATF_ATTR_S *pTemp = NULL;

	ret = isp_tnr_ctrl_get_tnr_matf_attr(ViPipe, &pTemp);
	if (pTemp != NULL) {
		memcpy(pstTNRMATFAttr, pTemp, sizeof(ISP_TNR_MATF_ATTR_S));
	}
...
CVI_S32 CVI_ISP_SetRgbirAttr(VI_PIPE ViPipe, const ISP_RGBIR_ATTR_S *pstRgbirAttr)
{
	...
	ret = isp_rgbir_ctrl_set_rgbir_attr(ViPipe, pstRgbirAttr);
...
CVI_S32 CVI_ISP_GetRgbirAttr(VI_PIPE ViPipe, ISP_RGBIR_ATTR_S *pstRgbirAttr)
{
	...
	CVI_S32 ret = CVI_SUCCESS;
	const ISP_RGBIR_ATTR_S *pTemp = NULL;

	ret = isp_rgbir_ctrl_get_rgbir_attr(ViPipe, &pTemp);
	if (pTemp != NULL) {
		memcpy(pstRgbirAttr, pTemp, sizeof(ISP_RGBIR_ATTR_S));
	}
...
{
	ISP_LOG_DEBUG("+\n");
	if (pstStitchAttr == CVI_NULL) {
		ISP_LOG_ERR("ViPipe %d pstStitchAttr = NULL\n", ViPipe);
		return CVI_FAILURE;
	}
	if ((ViPipe < 0) || (ViPipe >= VI_MAX_PIPE_NUM) ||
		pstStitchAttr->u8Group >= MAX_STITCH_GROUP) {
		ISP_LOG_ERR("ViPipe %d or Grop %d error\n", ViPipe, pstStitchAttr->u8Group);
		return -ENODEV;
	}
	...
	memcpy(&pstIspCtx->stStitchAttr, pstStitchAttr, sizeof(ISP_STITCH_ATTR_S));
...
CVI_S32 CVI_ISP_GetStitchAttr(VI_PIPE ViPipe, ISP_STITCH_ATTR_S *pstStitchAttr)
{
	...
	ISP_GET_CTX(ViPipe, pstIspCtx);
	memcpy(pstStitchAttr, &pstIspCtx->stStitchAttr, sizeof(ISP_STITCH_ATTR_S));
...
CVI_S32 CVI_ISP_StitchCalibartion(VI_PIPE ViPipe, ISP_STITCH_ATTR_S *pstAttr, CVI_U8 chnNum)
{
	ISP_LOG_DEBUG("+\n");
	if (pstAttr == CVI_NULL) {
		ISP_LOG_ERR("ViPipe %d pstAttr = NULL\n", ViPipe);
		return CVI_FAILURE;
	}

	if ((ViPipe < 0) || (ViPipe >= VI_MAX_PIPE_NUM) ||
			chnNum > VI_MAX_PIPE_NUM || pstAttr[ViPipe].u8Group >= MAX_STITCH_GROUP) {
		ISP_LOG_ERR("ViPipe:%d / chnNum:%d /group:%d value error\n",
						ViPipe, chnNum, pstAttr[ViPipe].u8Group);
		return -ENODEV;
	}

	return isp_3aLib_stitch_calibration(ViPipe, pstAttr, chnNum);
}

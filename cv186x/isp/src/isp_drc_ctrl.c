...
enum drc_tuning_toneCurve {
	...
	if (is_postprocess_update == CVI_FALSE) {
		...
		//debug setting
		if (drc_attr->DRCMu[DRC_DBG_DISABLE_LTM] == 1) {
			//disable dark tone
			drc_cfg->dark_enh_en = 0;
		} else if (drc_attr->DRCMu[DRC_DBG_DISABLE_LTM] == 2) {
			//disable bright tone
			drc_cfg->brit_enh_en = 0;
		}

		//dump curve
		CVI_U32 tunIdx = drc_attr->DRCMu[DRC_DBG_DUMP_CURVE];
		...
		//show lmap directly
		if (drc_attr->DRCMu[DRC_DBG_SHOW_LMAP] == 1 || drc_attr->DRCMu[DRC_DBG_SHOW_LMAP] == 2) {
			rng = 0;
			...
			drc_cfg->dbg_mode = drc_attr->DRCMu[DRC_DBG_SHOW_LMAP];
		}

		//rgbgamma set bypass
		if (drc_attr->DRCMu[DRC_DBG_RGBGAMMA_BYPASS] == 1) {

...
static CVI_S32 isp_bnr_ctrl_preprocess(VI_PIPE ViPipe, ISP_ALGO_RESULT_S *algoResult)
{
	...
	if ((!bnr_attr->Enable && !bnr_attr->EnableSE) || runtime->is_module_bypass)
		return ret;
	...
	if (bnr_attr->enOpType == OP_TYPE_MANUAL) {
		...
		MANUAL(bnr_attr, NoiseSuppressStrSE);
		MANUAL(bnr_attr, FilterTypeSE);
		MANUAL(bnr_attr, NoiseSuppressStrModeSE);
		...
		AUTO(bnr_attr, NoiseSuppressStrSE, INTPLT_POST_ISO);
		AUTO(bnr_attr, FilterTypeSE, INTPLT_POST_ISO);
		AUTO(bnr_attr, NoiseSuppressStrModeSE, INTPLT_POST_ISO);
	...
	runtime->bnr_param_in.filter_type_se = runtime->bnr_attr.FilterTypeSE;
...
static CVI_S32 isp_bnr_ctrl_postprocess(VI_PIPE ViPipe)
{
	...
	struct cvi_vip_isp_bnr_config *bnr_cfg_1 =
		&(post_addr->tun_cfg[tun_idx].bnr_cfg[1]);
	...
	if (IS_2to1_WDR_MODE(pstIspCtx->u8SnsWDRMode)) {
		if (is_postprocess_update == CVI_FALSE) {
			...
			memcpy(bnr_cfg_1, bnr_cfg, sizeof(struct cvi_vip_isp_bnr_config));
			bnr_cfg_1->enable = bnr_attr->EnableSE && !runtime->is_module_bypass;
			memcpy(bnr_cfg_1->weight_lut, runtime->bnr_param_out.weight_lut_se, sizeof(CVI_U8) * 256);
			bnr_cfg_1->bnr_1_cfg.NS_GAIN.bits.BNR_NS_GAIN = runtime->bnr_attr.NoiseSuppressStrSE;
			bnr_cfg_1->bnr_1_cfg.STRENGTH_MODE.bits.BNR_STRENGTH_MODE = runtime->bnr_attr.NoiseSuppressStrModeSE;
		}
	} else {
		bnr_cfg_1->enable = 0;
		bnr_cfg_1->update = 0;

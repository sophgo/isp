...
static CVI_S32 isp_rgbcac_ctrl_postprocess(VI_PIPE ViPipe)
{
	...
	struct cvi_vip_isp_rgbcac_config *rgbcac_cfg_1 =
		&(post_addr->tun_cfg[tun_idx].rgbcac_cfg[1]);

	ISP_GET_CTX(ViPipe, pstIspCtx);
	if (IS_2to1_WDR_MODE(pstIspCtx->u8SnsWDRMode)) {
		if (is_postprocess_update == CVI_FALSE) {
			rgbcac_cfg_1->update = 0;
		} else {
			memcpy(rgbcac_cfg_1, rgbcac_cfg, sizeof(struct cvi_vip_isp_rgbcac_config));
		}
	} else {
		rgbcac_cfg_1->enable = 0;
		rgbcac_cfg_1->update = 0;
	}

...
#define STS_U16_MAX (65535)
#define STS_U32_MAX (4294967295)
	...
	for (CVI_U32 chn = 0; chn < ISP_CHANNEL_MAX_NUM; chn++) {
		if (ae_sts[chn]->vir_addr && ae_sts_out) {
			...
			if(isTileMode(ViPipe)) {
				ISP_AE_STATISTICS_COMPAT_S ae_sts_out_right;
				ISP_WB_STATISTICS_S awb_sts_right;
				CVI_U32 addrOffset = ae_sts[chn]->size / 2;
				...
				isp_sts_ctrl_unpack_ae_sts(ViPipe, ae_sts[chn]->vir_addr + addrOffset, &ae_sts_out_right, &awb_sts_right, chn, 0);
				isp_sts_ctrl_unpack_ae_hist_sts(ViPipe, (ae_sts[chn]->vir_addr + hist_ofs + addrOffset), &ae_sts_out_right, chn, 0);

				for (CVI_U32 i= 0; i < MAX_HIST_BINS; i++) {
					tmpValue = ae_sts_out->aeStat1[0].au32HistogramMemArray[chn][i]
						+ ae_sts_out_right.aeStat1[0].au32HistogramMemArray[chn][i];

					ae_sts_out->aeStat1[0].au32HistogramMemArray[chn][i] =
						MIN2(tmpValue, STS_U32_MAX);
				}

				for (CVI_U8 ridx = 0 ; ridx < AE_ZONE_ROW ; ridx++) {
					for (CVI_U8 cidx = 0 ; cidx < AE_ZONE_COLUMN; cidx++) {
						CVI_U16 *sts_info = NULL;
						CVI_U16 *sts_info1 = NULL;
						CVI_U16 *sts_info2 = NULL;
						CVI_U16 idx, idx1, idx2;

						if (cidx < AE_ZONE_COLUMN / 2) {
							//ae
							sts_info =
								(CVI_U16 *)&(ae_sts_out->aeStat3[0].au16ZoneAvg[chn][ridx][cidx]);
							sts_info1 =
								(CVI_U16 *)&(ae_sts_out->aeStat3[0].au16ZoneAvg[chn][ridx][cidx * 2]);
							sts_info2 =
								(CVI_U16 *)&(ae_sts_out->aeStat3[0].au16ZoneAvg[chn][ridx][cidx * 2 + 1]);
							tmpValue = (sts_info1[0] + sts_info2[0]) / 2;
							sts_info[0] = MIN2(tmpValue, STS_U16_MAX);
							tmpValue = (sts_info1[1] + sts_info2[1]) / 2;
							sts_info[1] = MIN2(tmpValue, STS_U16_MAX);
							tmpValue = (sts_info1[2] + sts_info2[2]) / 2;
							sts_info[2] = MIN2(tmpValue, STS_U16_MAX);
							tmpValue = (sts_info1[3] + sts_info2[3]) / 2;
							sts_info[3] = MIN2(tmpValue, STS_U16_MAX);

							sts_info =
								(CVI_U16 *)&(ae_sts_out_right.aeStat3[0].au16ZoneAvg[chn][ridx][cidx]);
							sts_info1 =
								(CVI_U16 *)&(ae_sts_out_right.aeStat3[0].au16ZoneAvg[chn][ridx][cidx * 2]);
							sts_info2 =
								(CVI_U16 *)&(ae_sts_out_right.aeStat3[0].au16ZoneAvg[chn][ridx][cidx * 2 + 1]);
							tmpValue = (sts_info1[0] + sts_info2[0]) / 2;
							sts_info[0] = MIN2(tmpValue, STS_U16_MAX);
							tmpValue = (sts_info1[1] + sts_info2[1]) / 2;
							sts_info[1] = MIN2(tmpValue, STS_U16_MAX);
							...
							//awb
							idx = ridx * AE_ZONE_ROW + cidx;
							idx1 = ridx * AE_ZONE_ROW + cidx * 2;
							idx2 = ridx * AE_ZONE_ROW + cidx * 2 + 1;

							tmpValue = (awb_sts[chn].au16ZoneAvgB[idx1]
									+ awb_sts[chn].au16ZoneAvgB[idx2]) / 2;
							awb_sts[chn].au16ZoneAvgB[idx] =
								MIN2(tmpValue, STS_U16_MAX);
							tmpValue = (awb_sts[chn].au16ZoneAvgG[idx1]
								+ awb_sts[chn].au16ZoneAvgG[idx2]) / 2;
							awb_sts[chn].au16ZoneAvgG[idx] =
								MIN2(tmpValue, STS_U16_MAX);
							tmpValue = (awb_sts[chn].au16ZoneAvgR[idx1]
									+ awb_sts[chn].au16ZoneAvgR[idx2]) / 2;
							awb_sts[chn].au16ZoneAvgR[idx] =
								MIN2(tmpValue, STS_U16_MAX);

							tmpValue = (awb_sts_right.au16ZoneAvgB[idx1]
									+ awb_sts_right.au16ZoneAvgB[idx2]) / 2;
							awb_sts_right.au16ZoneAvgB[idx] =
								MIN2(tmpValue, STS_U16_MAX);
							tmpValue = (awb_sts_right.au16ZoneAvgG[idx1]
									+ awb_sts_right.au16ZoneAvgG[idx2]) / 2;
							awb_sts_right.au16ZoneAvgG[idx] =
								MIN2(tmpValue, STS_U16_MAX);
							tmpValue = (awb_sts_right.au16ZoneAvgR[idx1]
									+ awb_sts_right.au16ZoneAvgR[idx2]) / 2;
							awb_sts_right.au16ZoneAvgR[idx] =
								MIN2(tmpValue, STS_U16_MAX);
						} else {
							//ae
							sts_info =
								(CVI_U16 *)&(ae_sts_out->aeStat3[0].au16ZoneAvg[chn][ridx][cidx]);
							sts_info1 =
								(CVI_U16 *)&(ae_sts_out_right.aeStat3[0].au16ZoneAvg[chn][ridx][cidx - AE_ZONE_COLUMN / 2]);
							sts_info[0] = sts_info1[0];
							sts_info[1] = sts_info1[1];
							sts_info[2] = sts_info1[2];
							sts_info[3] = sts_info1[3];

							//awb
							idx = ridx * AE_ZONE_ROW + cidx;
							idx1 = ridx * AE_ZONE_ROW + cidx - AE_ZONE_COLUMN / 2;
							awb_sts[chn].au16ZoneAvgB[idx] = awb_sts_right.au16ZoneAvgB[idx1];
							awb_sts[chn].au16ZoneAvgG[idx] = awb_sts_right.au16ZoneAvgG[idx1];
							awb_sts[chn].au16ZoneAvgR[idx] = awb_sts_right.au16ZoneAvgR[idx1];
						}
					}
				}

				tmpValue = (ae_sts_out->aeStat2[0].u16GlobalAvgB[chn]
						+ ae_sts_out_right.aeStat2[0].u16GlobalAvgB[chn]) / 2;
				ae_sts_out->aeStat2[0].u16GlobalAvgB[chn] =
						MIN2(tmpValue, STS_U16_MAX);
				tmpValue = (ae_sts_out->aeStat2[0].u16GlobalAvgGr[chn]
						+ ae_sts_out_right.aeStat2[0].u16GlobalAvgGr[chn]) / 2;
				ae_sts_out->aeStat2[0].u16GlobalAvgGr[chn] =
						MIN2(tmpValue, STS_U16_MAX);
				tmpValue = (ae_sts_out->aeStat2[0].u16GlobalAvgGb[chn]
						+ ae_sts_out_right.aeStat2[0].u16GlobalAvgGb[chn]) / 2;
				ae_sts_out->aeStat2[0].u16GlobalAvgGb[chn] =
						MIN2(tmpValue, STS_U16_MAX);
				tmpValue = (ae_sts_out->aeStat2[0].u16GlobalAvgR[chn]
						+ ae_sts_out_right.aeStat2[0].u16GlobalAvgR[chn]) / 2;
				ae_sts_out->aeStat2[0].u16GlobalAvgR[chn] =
						MIN2(tmpValue, STS_U16_MAX);

				tmpValue = (awb_sts[chn].u16GlobalB + awb_sts_right.u16GlobalB) / 2;
				awb_sts[chn].u16GlobalB = MIN2(tmpValue, STS_U16_MAX);
				tmpValue = (awb_sts[chn].u16GlobalG + awb_sts_right.u16GlobalG) / 2;
				awb_sts[chn].u16GlobalG = MIN2(tmpValue, STS_U16_MAX);
				tmpValue = (awb_sts[chn].u16GlobalR + awb_sts_right.u16GlobalR) / 2;
				awb_sts[chn].u16GlobalR = MIN2(tmpValue, STS_U16_MAX);
			} else {
				isp_sts_ctrl_unpack_ae_sts(ViPipe, ae_sts[chn]->vir_addr, ae_sts_out, &(awb_sts[chn]), chn, 0);
...
static CVI_S32 isp_sts_ctrl_set_af_sts(VI_PIPE ViPipe)
{
	...
	if (af_sts_in && af_sts_out) {
		...
		if (isTileMode(ViPipe)) {
			ISP_AF_STATISTICS_S outRight;
			CVI_U32 addrOffset = sts_mem->af.size / 2;

			isp_sts_ctrl_unpack_af_sts(ViPipe, af_sts_in, af_sts_out);
			isp_sts_ctrl_unpack_af_sts(ViPipe, af_sts_in + addrOffset, &outRight);

			for (CVI_U32 i = 0; i < AF_ZONE_ROW; i++) {
				for (CVI_U32 j = 0; j < AF_ZONE_COLUMN; j++) {
					CVI_U64 cnt = 0, h0 = 0, h1 = 0, v0 = 0;
					CVI_U32 idx0 = 2 * j;
					CVI_U32 idx1 = 2 * j + 1;

					if (idx0 >= AF_ZONE_COLUMN) {
						idx0 -= AF_ZONE_COLUMN;
						cnt += outRight.stFEAFStat.stZoneMetrics[i][idx0].u16HlCnt;
						h0 += outRight.stFEAFStat.stZoneMetrics[i][idx0].u64h0;
						h1 += outRight.stFEAFStat.stZoneMetrics[i][idx0].u64h1;
						v0 += outRight.stFEAFStat.stZoneMetrics[i][idx0].u32v0;
					} else {
						cnt += af_sts_out->stFEAFStat.stZoneMetrics[i][idx0].u16HlCnt;
						h0 += af_sts_out->stFEAFStat.stZoneMetrics[i][idx0].u64h0;
						h1 += af_sts_out->stFEAFStat.stZoneMetrics[i][idx0].u64h1;
						v0 += af_sts_out->stFEAFStat.stZoneMetrics[i][idx0].u32v0;
					}
					if (idx1 >= AF_ZONE_COLUMN) {
						idx1 -= AF_ZONE_COLUMN;
						cnt += outRight.stFEAFStat.stZoneMetrics[i][idx1].u16HlCnt;
						h0 += outRight.stFEAFStat.stZoneMetrics[i][idx1].u64h0;
						h1 += outRight.stFEAFStat.stZoneMetrics[i][idx1].u64h1;
						v0 += outRight.stFEAFStat.stZoneMetrics[i][idx1].u32v0;
					} else {
						cnt += af_sts_out->stFEAFStat.stZoneMetrics[i][idx1].u16HlCnt;
						h0 += af_sts_out->stFEAFStat.stZoneMetrics[i][idx1].u64h0;
						h1 += af_sts_out->stFEAFStat.stZoneMetrics[i][idx1].u64h1;
						v0 += af_sts_out->stFEAFStat.stZoneMetrics[i][idx1].u32v0;
					}

					af_sts_out->stFEAFStat.stZoneMetrics[i][j].u16HlCnt =
						MIN2(cnt / 2, STS_U16_MAX);
					af_sts_out->stFEAFStat.stZoneMetrics[i][j].u64h0 =
						MIN2(h0 / 2, STS_U32_MAX);
					af_sts_out->stFEAFStat.stZoneMetrics[i][j].u64h1 =
						MIN2(h1 / 2, STS_U32_MAX);
					af_sts_out->stFEAFStat.stZoneMetrics[i][j].u32v0 =
						MIN2(v0 / 2, STS_U32_MAX);
				}
			}
...
static CVI_S32 isp_sts_ctrl_set_dci_sts(VI_PIPE ViPipe)
{
	...
	if (dci_sts_in && dci_sts_out) {
		if (isTileMode(ViPipe)) {
			CVI_U32 hist_right[DCI_BINS_NUM];
			CVI_U32 size = runtime->sts_mem[runtime->post_sts_idx].dci.size;

			memcpy(dci_sts_out, dci_sts_in, sizeof(CVI_U32) * DCI_BINS_NUM);
			memcpy(hist_right, dci_sts_in + size / 2 , sizeof(CVI_U32) * DCI_BINS_NUM);
			for (CVI_U32 i = 0; i < DCI_BINS_NUM; i++) {
				CVI_U64 tmpValue = dci_sts_out->hist[i] + hist_right[i];

				dci_sts_out->hist[i] = MIN2(tmpValue, STS_U32_MAX);
			}
		} else {
			memcpy(dci_sts_out, dci_sts_in, sizeof(CVI_U32) * DCI_BINS_NUM);
		}
...
static CVI_S32 isp_sts_ctrl_set_gms_sts(VI_PIPE ViPipe)
{
	...
	if (gms_sts_in && gms_sts_out) {
		...
		if (isTileMode(ViPipe)) {
			ISP_DIS_STATS_INFO outRight;
			CVI_U32 addrOffset = sts_mem->gms.size / 2;

			isp_sts_ctrl_unpack_gms_sts(gms_sts_out, gms_sts_in);
			isp_sts_ctrl_unpack_gms_sts(&outRight, gms_sts_in + addrOffset);

			for (CVI_U32 i = 0; i < DIS_MAX_WINDOW_X_NUM; i++) {
				for (CVI_U32 j = 0; j < DIS_MAX_WINDOW_Y_NUM; j++) {
					CVI_U16 *ptrX = NULL;
					CVI_U16 *ptrY = NULL;
					CVI_U16 idx = (i % 2 == 0) ? (2 * j) : (2 * j + 1);

					if (idx >= DIS_MAX_WINDOW_Y_NUM) {
						ptrX = outRight.histX[i][idx - DIS_MAX_WINDOW_Y_NUM];
						ptrY = outRight.histY[i][idx - DIS_MAX_WINDOW_Y_NUM];
					} else {
						ptrX = gms_sts_out->histX[i][idx];
						ptrY = gms_sts_out->histY[i][idx];
					}

					if (ptrX != gms_sts_out->histX[i][j])
						memcpy(gms_sts_out->histX[i][j], ptrX, sizeof(CVI_U16) * XHIST_LENGTH);
					if (ptrY != gms_sts_out->histY[i][j])
						memcpy(gms_sts_out->histY[i][j], ptrY, sizeof(CVI_U16) * YHIST_LENGTH);
				}
			}
		} else {
			isp_sts_ctrl_unpack_gms_sts(gms_sts_out, gms_sts_in);
...
static CVI_S32 isp_sts_ctrl_set_ae_cfg(VI_PIPE ViPipe)
{
	...
	CVI_U16 u16W, u16H, Width;
	CVI_U32 sts_div_temp;

	ae_cfg->update = 1;
	ae_cfg->ae_enable = 1;
	Width = pstIspCtx->stSysRect.u32Width;
	...
	if (ae_attr->stCrop[0].bEnable) {
		u16W = Width - ae_attr->stCrop[0].u16X;
		...
		ae_cfg->ae_offsetx = 0;
		ae_cfg->ae_offsety = 0;
		ae_cfg->ae_width = (((Width / ae_cfg->ae_numx) >> 1) << 1);
...
static CVI_S32 isp_sts_ctrl_set_af_cfg(VI_PIPE ViPipe)
{
	...
	CVI_U32 width = pstIspCtx->stSysRect.u32Width;
	...
	if (pstIspCtx->stsCfgInfo.stFocusCfg.stConfig.stCrop.bEnable) {
		...
		af_cfg->block_width = ((width - (AF_XOFFSET_MIN << 1))
			/ af_cfg->block_numx) >> 1 << 1;

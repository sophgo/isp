...
static CVI_S32 isp_demosaic_ctrl_preprocess(VI_PIPE ViPipe, ISP_ALGO_RESULT_S *algoResult)
{
	...
	if (demosaic_attr->enOpType == OP_TYPE_MANUAL) {
		...
		MANUAL(demosaic_demoire_attr, SharpenGain);
		...
		AUTO(demosaic_demoire_attr, SharpenGain, INTPLT_POST_ISO);
...
static CVI_S32 isp_demosaic_ctrl_postprocess(VI_PIPE ViPipe)
{
	...
	if (is_postprocess_update == CVI_FALSE) {
		...
		cfg_1->REG_190.bits.CFA_SHPN_GAIN_VALUE = runtime->demosaic_demoire_attr.SharpenGain;
	...
	struct cvi_vip_isp_demosiac_config *demosaic_cfg_01 = {
		&(post_addr->tun_cfg[tun_idx].demosiac_cfg[1])
	};

	ISP_GET_CTX(ViPipe, pstIspCtx);
	if (IS_2to1_WDR_MODE(pstIspCtx->u8SnsWDRMode)) {
		if (is_postprocess_update == CVI_FALSE) {
			demosaic_cfg_01->update = 0;
		} else {
			memcpy(demosaic_cfg_01, demosaic_cfg, sizeof(struct cvi_vip_isp_demosiac_config));
		}
	} else {
		demosaic_cfg_01->cfa_enable = 0;
		demosaic_cfg_01->update = 0;
	}

	demosaic_cfg_01->inst = 1;
	...
	const ISP_DEMOSAIC_ATTR_S *pstDemosaicAttr)
{
	...
	if (pstDemosaicAttr->Enable == CVI_FALSE) {

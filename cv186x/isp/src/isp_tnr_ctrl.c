...
static CVI_S32 isp_tnr_ctrl_check_tnr_matf_attr_valid(const ISP_TNR_MATF_ATTR_S *pstTNRMATFAttr);
...
static CVI_S32 isp_tnr_ctrl_preprocess(VI_PIPE ViPipe, ISP_ALGO_RESULT_S *algoResult)
{
	...
	const ISP_TNR_MATF_ATTR_S *tnr_matf_attr = NULL;
	...
	isp_tnr_ctrl_get_tnr_matf_attr(ViPipe, &tnr_matf_attr);
	...
	if (tnr_attr->enOpType == OP_TYPE_MANUAL) {
		...
		MANUAL(tnr_matf_attr, Enable);
		if (runtime->tnr_matf_attr.Enable) {
			for (CVI_U32 i = 0 ; i < 4 ; i++) {
				MANUAL(tnr_matf_attr, LowMtPrtAdvIn[i]);
				MANUAL(tnr_matf_attr, LowMtPrtAdvOut[i]);
			}
			MANUAL(tnr_matf_attr, MaxTFNum);
			MANUAL(tnr_matf_attr, MinNoiseLevel);
			MANUAL(tnr_matf_attr, MaxNoiseLevel);
			MANUAL(tnr_matf_attr, PixelDiffMotionOutThr);
			MANUAL(tnr_matf_attr, Vec10);
			MANUAL(tnr_matf_attr, Vec20);
			MANUAL(tnr_matf_attr, Vec30);
		} else {
			// TODO: mason.zou
		}
		...
		AUTO(tnr_matf_attr, Enable, INTPLT_POST_ISO);
		if (runtime->tnr_matf_attr.Enable) {
			for (CVI_U32 i = 0 ; i < 4 ; i++) {
				AUTO(tnr_matf_attr, LowMtPrtAdvIn[i], INTPLT_POST_ISO);
				AUTO(tnr_matf_attr, LowMtPrtAdvOut[i], INTPLT_POST_ISO);
			}
			AUTO(tnr_matf_attr, MaxTFNum, INTPLT_POST_ISO);
			AUTO(tnr_matf_attr, MinNoiseLevel, INTPLT_POST_ISO);
			AUTO(tnr_matf_attr, MaxNoiseLevel, INTPLT_POST_ISO);
			AUTO(tnr_matf_attr, PixelDiffMotionOutThr, INTPLT_POST_ISO);
			AUTO(tnr_matf_attr, Vec10, INTPLT_POST_ISO);
			AUTO(tnr_matf_attr, Vec20, INTPLT_POST_ISO);
			AUTO(tnr_matf_attr, Vec30, INTPLT_POST_ISO);
		} else {
			// TODO: mason.zou
		}
	...
	for (CVI_U32 i = 0 ; i < 4 ; i++) {
		...
		if (runtime->tnr_matf_attr.Enable) {
			runtime->tnr_param_in.LowMtPrtAdvIn[i] = runtime->tnr_matf_attr.LowMtPrtAdvIn[i];
			runtime->tnr_param_in.LowMtPrtAdvOut[i] = runtime->tnr_matf_attr.LowMtPrtAdvOut[i];
		} else {
...
static CVI_S32 isp_tnr_ctrl_postprocess(VI_PIPE ViPipe)
{
	...
	if (is_postprocess_update == CVI_FALSE) {
		...
		if (tnr_attr->Enable && !runtime->is_module_bypass) {
			if (runtime->tnr_matf_attr.Enable) {
				tnr_cfg->manr_enable = 0x02; // MATF (new algo)
			} else {
				tnr_cfg->manr_enable = 0x01; // NRMA (same as mars)
			}
		} else {
			tnr_cfg->manr_enable = 0x00; // bypass
		}
		...
		cfg_6->REG_F4.bits.TDNR_JND_THS = 125;
		cfg_6->REG_F4.bits.TDNR_JND_SAT = 250;
		cfg_6->REG_F4.bits.TDNR_JND_GAIN_RSB = 4;
		cfg_6->REG_F4.bits.TDNR_MAX_TF_NUM = runtime->tnr_matf_attr.MaxTFNum;
		cfg_6->REG_F8.bits.TDNR_MAX_NOISE_LEVEL = runtime->tnr_matf_attr.MaxNoiseLevel;
		cfg_6->REG_F8.bits.TDNR_MIN_NOISE_LEVEL = runtime->tnr_matf_attr.MinNoiseLevel;
		cfg_6->REG_FC.bits.TDNR_PIXEL_DIFF_MOTIONOUT = runtime->tnr_matf_attr.PixelDiffMotionOutThr;
		cfg_6->REG_FC.bits.TDNR_VEC1_0 = runtime->tnr_matf_attr.Vec10;
		cfg_6->REG_FC.bits.TDNR_VEC2_0 = runtime->tnr_matf_attr.Vec20;
		cfg_6->REG_FC.bits.TDNR_VEC3_0 = runtime->tnr_matf_attr.Vec30;
...
CVI_S32 isp_tnr_ctrl_get_tnr_matf_attr(VI_PIPE ViPipe,
			const ISP_TNR_MATF_ATTR_S **pstTNRMATFAttr)
{
	...
	isp_mgr_buf_get_addr(ViPipe, ISP_IQ_BLOCK_3DNR, (CVI_VOID *) &shared_buffer);
	*pstTNRMATFAttr = &shared_buffer->stTNRMATFAttr;

	return ret;
}

CVI_S32 isp_tnr_ctrl_set_tnr_matf_attr(VI_PIPE ViPipe, const ISP_TNR_MATF_ATTR_S *pstTNRMATFAttr)
{
	if (pstTNRMATFAttr == CVI_NULL) {
		return CVI_FAILURE;
	}
	...
	ret = isp_tnr_ctrl_check_tnr_matf_attr_valid(pstTNRMATFAttr);
	if (ret != CVI_SUCCESS)
		return ret;

	const ISP_TNR_MATF_ATTR_S *p = CVI_NULL;

	isp_tnr_ctrl_get_tnr_matf_attr(ViPipe, &p);
	memcpy((CVI_VOID *)p, pstTNRMATFAttr, sizeof(*pstTNRMATFAttr));

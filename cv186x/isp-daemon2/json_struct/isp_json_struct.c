...
// DCI enumeration
// -----------------------------------------------------------------------------
static void ISP_DCI_CURVE_MODE_E_JSON(int r_w_flag, JSON *j, char *key, ISP_DCI_CURVE_MODE_E *value)
{
	JSON *obj = 0;
	...
	if (r_w_flag == R_FLAG) {
		if (cvi_json_object_object_get_ex2(j, key, &obj)) {
			...
			temp = cvi_json_object_get_int(obj);
			JSON_CHECK_RANGE(key, &temp, DCI_CURVE_AUTO, DCI_CURVE_MANUAL);
			*value = temp;
		} else {
			JSON_PRINT_ERR_NOT_EXIST(key);
		}
...
// -----------------------------------------------------------------------------
static void ISP_IRIS_PID_MODE_E_JSON(int r_w_flag, JSON *j, char *key, ISP_IRIS_PID_MODE_E *value)
{
	JSON *obj = 0;
	...
	if (r_w_flag == R_FLAG) {
		if (cvi_json_object_object_get_ex2(j, key, &obj)) {
			...
			temp = cvi_json_object_get_int(obj);
			JSON_CHECK_RANGE(key, &temp, ISP_IRIS_DELTA_PID, ISP_IRIS_POSITION_PID);
			*value = temp;
		} else {
			JSON_PRINT_ERR_NOT_EXIST(key);
		}
...
static void ISP_BAYER_FORMAT_E_JSON(int r_w_flag, JSON *j, char *key, ISP_BAYER_FORMAT_E *value)
{
	JSON *obj = 0;
	...
	if (r_w_flag == R_FLAG) {
		if (cvi_json_object_object_get_ex2(j, key, &obj)) {
			...
			temp = cvi_json_object_get_int(obj);
			JSON_CHECK_RANGE(key, &temp, BAYER_BGGR, BAYER_BUTT);
			*value = temp;
		} else {
			JSON_PRINT_ERR_NOT_EXIST(key);
		}
...
// ISP structure - rgbir
// -----------------------------------------------------------------------------
static void ISP_RGBIR_MANUAL_ATTR_S_JSON(int r_w_flag, JSON *j, char *key, ISP_RGBIR_MANUAL_ATTR_S *data)
{
	JSON_START(r_w_flag);

	JSON(r_w_flag, CVI_U16, u16RecGbrGain);
	JSON(r_w_flag, CVI_U16, u16RecGbrOffset);
	JSON(r_w_flag, CVI_U16, u16RecGirGain);
	JSON(r_w_flag, CVI_U16, u16RecGirOffset);
	JSON(r_w_flag, CVI_U16, u16RecRgGain);
	JSON(r_w_flag, CVI_U16, u16RecRgOffset);
	JSON(r_w_flag, CVI_U16, u16RecBgGain);
	JSON(r_w_flag, CVI_U16, u16RecBgOffset);

	JSON_END(r_w_flag);
}

// -----------------------------------------------------------------------------
static void ISP_RGBIR_AUTO_ATTR_S_JSON(int r_w_flag, JSON *j, char *key, ISP_RGBIR_AUTO_ATTR_S *data)
{
	JSON_START(r_w_flag);

	JSON_A(r_w_flag, CVI_U16, u16RecGbrGain, ISP_AUTO_ISO_STRENGTH_NUM);
	JSON_A(r_w_flag, CVI_U16, u16RecGbrOffset, ISP_AUTO_ISO_STRENGTH_NUM);
	JSON_A(r_w_flag, CVI_U16, u16RecGirGain, ISP_AUTO_ISO_STRENGTH_NUM);
	JSON_A(r_w_flag, CVI_U16, u16RecGirOffset, ISP_AUTO_ISO_STRENGTH_NUM);
	JSON_A(r_w_flag, CVI_U16, u16RecRgGain, ISP_AUTO_ISO_STRENGTH_NUM);
	JSON_A(r_w_flag, CVI_U16, u16RecRgOffset, ISP_AUTO_ISO_STRENGTH_NUM);
	JSON_A(r_w_flag, CVI_U16, u16RecBgGain, ISP_AUTO_ISO_STRENGTH_NUM);
	JSON_A(r_w_flag, CVI_U16, u16RecBgOffset, ISP_AUTO_ISO_STRENGTH_NUM);

	JSON_END(r_w_flag);
}

// -----------------------------------------------------------------------------
void ISP_RGBIR_ATTR_S_JSON(int r_w_flag, JSON *j, char *key, ISP_RGBIR_ATTR_S *data)
{
	JSON_START(r_w_flag);
	...
	JSON(r_w_flag, ISP_OP_TYPE_E, enOpType);
	JSON(r_w_flag, CVI_U8, UpdateInterval);
	JSON(r_w_flag, ISP_BAYER_FORMAT_E, enIrBayerFormat);
	JSON(r_w_flag, CVI_BOOL, bLumaCompEnable);
	JSON(r_w_flag, CVI_U8, u8RCompGain);
	JSON(r_w_flag, CVI_U8, u8GCompGain);
	JSON(r_w_flag, CVI_U8, u8BCompGain);
	JSON(r_w_flag, CVI_U8, u8DebugMode);
	JSON(r_w_flag, ISP_RGBIR_MANUAL_ATTR_S, stManual);
	JSON(r_w_flag, ISP_RGBIR_AUTO_ATTR_S, stAuto);
...
static void ISP_DP_DYNAMIC_MANUAL_ATTR_S_JSON(int r_w_flag, JSON *j, char *key, ISP_DP_DYNAMIC_MANUAL_ATTR_S *data)
{
	...
	JSON(r_w_flag, CVI_U16, BrightDefectToNormalPixRatio);
	JSON(r_w_flag, CVI_U16, DarkDefectToNormalPixRatio);
...
static void ISP_DP_DYNAMIC_AUTO_ATTR_S_JSON(int r_w_flag, JSON *j, char *key, ISP_DP_DYNAMIC_AUTO_ATTR_S *data)
{
	...
	JSON_A(r_w_flag, CVI_U16, BrightDefectToNormalPixRatio, ISP_AUTO_ISO_STRENGTH_NUM);
	JSON_A(r_w_flag, CVI_U16, DarkDefectToNormalPixRatio, ISP_AUTO_ISO_STRENGTH_NUM);
...
static void ISP_NR_MANUAL_ATTR_S_JSON(int r_w_flag, JSON *j, char *key, ISP_NR_MANUAL_ATTR_S *data)
{
	...
	JSON(r_w_flag, CVI_U8, NoiseSuppressStrSE);
	JSON(r_w_flag, CVI_U8, FilterTypeSE);
	JSON(r_w_flag, CVI_U8, NoiseSuppressStrModeSE);
...
static void ISP_NR_AUTO_ATTR_S_JSON(int r_w_flag, JSON *j, char *key, ISP_NR_AUTO_ATTR_S *data)
{
	...
	JSON_A(r_w_flag, CVI_U8, NoiseSuppressStrSE, ISP_AUTO_ISO_STRENGTH_NUM);
	JSON_A(r_w_flag, CVI_U8, FilterTypeSE, ISP_AUTO_ISO_STRENGTH_NUM);
	JSON_A(r_w_flag, CVI_U8, NoiseSuppressStrModeSE, ISP_AUTO_ISO_STRENGTH_NUM);
...
void ISP_NR_ATTR_S_JSON(int r_w_flag, JSON *j, char *key, ISP_NR_ATTR_S *data)
{
	...
	JSON(r_w_flag, CVI_BOOL, Enable);
	JSON(r_w_flag, CVI_BOOL, EnableSE);
	JSON(r_w_flag, ISP_OP_TYPE_E, enOpType);
	JSON(r_w_flag, CVI_U8, UpdateInterval);
	...
	ISP_DEMOSAIC_DEMOIRE_MANUAL_ATTR_S *data)
{
	...
	JSON(r_w_flag, CVI_U8, SharpenGain);
	...
	ISP_DEMOSAIC_DEMOIRE_AUTO_ATTR_S *data)
{
	...
	JSON_A(r_w_flag, CVI_U8, SharpenGain, ISP_AUTO_ISO_STRENGTH_NUM);
...
void ISP_DCI_ATTR_S_JSON(int r_w_flag, JSON *j, char *key, ISP_DCI_ATTR_S *data)
{
	...
	JSON(r_w_flag, CVI_BOOL, Enable);
	JSON(r_w_flag, CVI_BOOL, TuningMode);
	JSON(r_w_flag, ISP_DCI_CURVE_MODE_E, CurveMode);
	JSON(r_w_flag, CVI_U8, Method);
	JSON(r_w_flag, CVI_U8, UpdateInterval);
	JSON(r_w_flag, CVI_U16, Speed);
	JSON(r_w_flag, CVI_U16, DciStrength);
	JSON(r_w_flag, CVI_U8, DciGamma);
	JSON(r_w_flag, CVI_U8, DciOffset);
	JSON(r_w_flag, CVI_U8, DciContrast);
	JSON_A(r_w_flag, CVI_U16, DciGammaCurve, DCI_BINS_NUM);
...
static void ISP_PRESHARPEN_MANUAL_ATTR_S_JSON(int r_w_flag, JSON *j, char *key, ISP_PRESHARPEN_MANUAL_ATTR_S *data)
{
	...
	JSON(r_w_flag, CVI_U8, MFCoreGain);
	JSON(r_w_flag, CVI_U8, GlobalGain);
	JSON(r_w_flag, CVI_U8, ShootMode);
	JSON(r_w_flag, CVI_U8, OverShootGain);
	JSON(r_w_flag, CVI_U8, UnderShootGain);
	...
	JSON(r_w_flag, CVI_U8, EnhanceMode);
	JSON(r_w_flag, CVI_U32, DarkLumaThr);
	JSON(r_w_flag, CVI_U32, BrightLumaThr);
	JSON(r_w_flag, CVI_U32, DarkEdgeThr);
	JSON(r_w_flag, CVI_U32, DarkEdgeStrength);
	JSON(r_w_flag, CVI_U32, BrightEdgeThr);
	JSON(r_w_flag, CVI_U32, BrightEdgeStrength);
	JSON(r_w_flag, CVI_U32, LineThr);
	JSON(r_w_flag, CVI_U32, LineStrength);
...
static void ISP_PRESHARPEN_AUTO_ATTR_S_JSON(int r_w_flag, JSON *j, char *key, ISP_PRESHARPEN_AUTO_ATTR_S *data)
{
	...
	JSON_A(r_w_flag, CVI_U8, MFCoreGain, ISP_AUTO_ISO_STRENGTH_NUM);
	JSON_A(r_w_flag, CVI_U8, GlobalGain, ISP_AUTO_ISO_STRENGTH_NUM);
	JSON_A(r_w_flag, CVI_U8, ShootMode, ISP_AUTO_ISO_STRENGTH_NUM);
	...
	JSON_A(r_w_flag, CVI_U8, EnhanceMode, ISP_AUTO_ISO_STRENGTH_NUM);
	JSON_A(r_w_flag, CVI_U32, DarkLumaThr, ISP_AUTO_ISO_STRENGTH_NUM);
	JSON_A(r_w_flag, CVI_U32, BrightLumaThr, ISP_AUTO_ISO_STRENGTH_NUM);
	JSON_A(r_w_flag, CVI_U32, DarkEdgeThr, ISP_AUTO_ISO_STRENGTH_NUM);
	JSON_A(r_w_flag, CVI_U32, DarkEdgeStrength, ISP_AUTO_ISO_STRENGTH_NUM);
	JSON_A(r_w_flag, CVI_U32, BrightEdgeThr, ISP_AUTO_ISO_STRENGTH_NUM);
	JSON_A(r_w_flag, CVI_U32, BrightEdgeStrength, ISP_AUTO_ISO_STRENGTH_NUM);
	JSON_A(r_w_flag, CVI_U32, LineThr, ISP_AUTO_ISO_STRENGTH_NUM);
	JSON_A(r_w_flag, CVI_U32, LineStrength, ISP_AUTO_ISO_STRENGTH_NUM);
...
static void ISP_TNR_GHOST_MANUAL_ATTR_S_JSON(int r_w_flag, JSON *j, char *key, ISP_TNR_GHOST_MANUAL_ATTR_S *data)
{
	...
	JSON(r_w_flag, CVI_U8, CompRatioThr);
	JSON(r_w_flag, CVI_U8, CompRatio);
...
static void ISP_TNR_GHOST_AUTO_ATTR_S_JSON(int r_w_flag, JSON *j, char *key, ISP_TNR_GHOST_AUTO_ATTR_S *data)
{
	...
	JSON_A(r_w_flag, CVI_U8, CompRatioThr, ISP_AUTO_ISO_STRENGTH_NUM);
	JSON_A(r_w_flag, CVI_U8, CompRatio, ISP_AUTO_ISO_STRENGTH_NUM);
...
// -----------------------------------------------------------------------------
static void ISP_TNR_MATF_MANUAL_ATTR_S_JSON(int r_w_flag, JSON *j, char *key,
	ISP_TNR_MATF_MANUAL_ATTR_S *data)
{
	JSON_START(r_w_flag);

	JSON(r_w_flag, CVI_U8, Enable);
	JSON_A(r_w_flag, CVI_U8, LowMtPrtAdvIn, 4);
	JSON_A(r_w_flag, CVI_U8, LowMtPrtAdvOut, 4);
	JSON(r_w_flag, CVI_U8, MaxTFNum);
	JSON(r_w_flag, CVI_U8, MinNoiseLevel);
	JSON(r_w_flag, CVI_U8, MaxNoiseLevel);
	JSON(r_w_flag, CVI_U8, PixelDiffMotionOutThr);
	JSON(r_w_flag, CVI_U8, Vec10);
	JSON(r_w_flag, CVI_U8, Vec20);
	JSON(r_w_flag, CVI_U8, Vec30);

	JSON_END(r_w_flag);
}

// -----------------------------------------------------------------------------
static void ISP_TNR_MATF_AUTO_ATTR_S_JSON(int r_w_flag, JSON *j, char *key,
	ISP_TNR_MATF_AUTO_ATTR_S *data)
{
	JSON_START(r_w_flag);

	JSON_A(r_w_flag, CVI_U8, Enable, ISP_AUTO_ISO_STRENGTH_NUM);
	JSON_A(r_w_flag, CVI_U8, LowMtPrtAdvIn, 4 * ISP_AUTO_ISO_STRENGTH_NUM);
	JSON_A(r_w_flag, CVI_U8, LowMtPrtAdvOut, 4 * ISP_AUTO_ISO_STRENGTH_NUM);
	JSON_A(r_w_flag, CVI_U8, MaxTFNum, ISP_AUTO_ISO_STRENGTH_NUM);
	JSON_A(r_w_flag, CVI_U8, MinNoiseLevel, ISP_AUTO_ISO_STRENGTH_NUM);
	JSON_A(r_w_flag, CVI_U8, MaxNoiseLevel, ISP_AUTO_ISO_STRENGTH_NUM);
	JSON_A(r_w_flag, CVI_U8, PixelDiffMotionOutThr, ISP_AUTO_ISO_STRENGTH_NUM);
	JSON_A(r_w_flag, CVI_U8, Vec10, ISP_AUTO_ISO_STRENGTH_NUM);
	JSON_A(r_w_flag, CVI_U8, Vec20, ISP_AUTO_ISO_STRENGTH_NUM);
	JSON_A(r_w_flag, CVI_U8, Vec30, ISP_AUTO_ISO_STRENGTH_NUM);

	JSON_END(r_w_flag);
}

// -----------------------------------------------------------------------------
void ISP_TNR_MATF_ATTR_S_JSON(int r_w_flag, JSON *j, char *key, ISP_TNR_MATF_ATTR_S *data)
{
	JSON_START(r_w_flag);

	JSON(r_w_flag, ISP_TNR_MATF_MANUAL_ATTR_S, stManual);
	JSON(r_w_flag, ISP_TNR_MATF_AUTO_ATTR_S, stAuto);
...
static void ISP_AE_ATTR_S_JSON(int r_w_flag, JSON *j, char *key, ISP_AE_ATTR_S *data)
{
	...
	JSON(r_w_flag, CVI_U16, u16AEStrategyThr);
...
void ISP_EXPOSURE_ATTR_S_JSON(int r_w_flag, JSON *j, char *key, ISP_EXPOSURE_ATTR_S *data)
{
	...
	JSON(r_w_flag, CVI_S8, s8SensorSensitivity);
...
void ISP_DCIRIS_ATTR_S_JSON(int r_w_flag, JSON *j, char *key, ISP_DCIRIS_ATTR_S *data)
{
	...
	JSON(r_w_flag, CVI_U32, u32MaxSafeLuma);
	JSON(r_w_flag, CVI_U32, u32ToSafePwmDuty);
	JSON(r_w_flag, ISP_IRIS_PID_MODE_E, enIrisPIDMode);
...
void ISP_PARAMETER_BUFFER_JSON(int r_w_flag, JSON *j, char *key_word, ISP_Parameter_Structures *data, int idx)
{
	...
	// YUV-Top
		...
		JSON(r_w_flag, ISP_TNR_MATF_ATTR_S, tnr_matf);

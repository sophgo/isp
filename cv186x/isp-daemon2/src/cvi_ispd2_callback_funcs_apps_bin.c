...
static enum CVI_BIN_SECTION_ID CVI_ISPD2_GetModuleIdFromString(const char *pszTemp)
{
	...
	if (strcmp(pszTemp, "All") == 0) {
		...
		id = CVI_BIN_ID_ISP3;
	} else if (strcmp(pszTemp, "Sensor_4") == 0) {
		id = CVI_BIN_ID_ISP4;
	} else if (strcmp(pszTemp, "Sensor_5") == 0) {
		id = CVI_BIN_ID_ISP5;
	} else if (strcmp(pszTemp, "Sensor_6") == 0) {
		id = CVI_BIN_ID_ISP6;
	} else if (strcmp(pszTemp, "Sensor_7") == 0) {
		id = CVI_BIN_ID_ISP7;
...
#ifdef ENABLE_ISP_IPC
	s32Ret = CVI_BIN_GetBinNameWrap(szBinFileName);
#else
	...
	TJSONRpcContentOut *ptContentOut, JSONObject *pJsonResponse)
{
	...
	VPSS_GRP VpssGrp = ptIn->s32VpssGrp;
...
#ifdef ENABLE_ISP_IPC
	s32Ret = CVI_BIN_ImportBinDataWrap(ptBinaryData->pu8Buffer, ptBinaryData->u32Size);
#else
	...
	CVI_VPSS_SetGrpParamfromBin(VpssGrp, VpssGrp);
	ptContentOut->s32StatusCode = JSONRPC_CODE_OK;

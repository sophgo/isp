...
static CVI_S32 CVI_ISPD2_GetRAWFrameInfo(const TGetFrameInfo *pstFrameInfo, TFrameHeader *pstFrameHeader)
{
	...
	compressMode = stChnAttr.enCompressMode;
	wdrMode = stDevAttr.stWDRAttr.enWDRMode;
	bayerFormat = stDevAttr.enBayerFormat;
#else
	...
	if (CVI_VI_SetPipeDumpAttr(pstFrameInfo->ViPipe, &stDumpAttr) != CVI_SUCCESS) {
		ISP_DAEMON2_DEBUG(LOG_ERR, "CVI_VI_SetPipeDumpAttr fail");
		return CVI_FAILURE;
	}

	for (CVI_U32 u32Idx = 0; u32Idx < MAX_WDR_FUSION_FRAMES; ++u32Idx) {
		astVideoFrame[u32Idx].stVFrame.enPixelFormat = PIXEL_FORMAT_RGB_BAYER_12BPP;
	}

	if (CVI_VI_GetPipeFrame(pstFrameInfo->ViPipe, astVideoFrame, MAX_GET_FRAME_TIMEOUT) != CVI_SUCCESS) {
		ISP_DAEMON2_DEBUG(LOG_ERR, "CVI_VI_GetPipeFrame fail (timeout)");
		return CVI_FAILURE;
	}

	if (CVI_VI_ReleasePipeFrame(pstFrameInfo->ViPipe, astVideoFrame) != CVI_SUCCESS) {
		ISP_DAEMON2_DEBUG(LOG_ERR, "CVI_VI_ReleasePipeFrame fail");
		return CVI_FAILURE;
	}

	if (CVI_ISP_GetPubAttr(pstFrameInfo->ViPipe, &stPubAttr) != CVI_SUCCESS) {
		ISP_DAEMON2_DEBUG(LOG_ERR, "Get Pub Attr fail");
		return CVI_FAILURE;
	}

	u32FrameWidth = astVideoFrame[0].stVFrame.u32Width;
	u32FrameHeight = astVideoFrame[0].stVFrame.u32Height;
	compressMode = astVideoFrame[0].stVFrame.enCompressMode;
	wdrMode = stPubAttr.enWDRMode;
	bayerFormat = astVideoFrame[0].stVFrame.enBayerFormat;
#endif

	u32BlkSize = VI_GetRawBufferSize(u32FrameWidth, u32FrameHeight,
		PIXEL_FORMAT_RGB_BAYER_12BPP, compressMode, DEFAULT_ALIGN,
		(u32FrameWidth > RAW_FRAME_TILE_MODE_WIDTH) ? CVI_TRUE : CVI_FALSE);
	u8WDRFusionFrame = CVI_ISPD2_GetWDRFusionFrame(wdrMode);
	...
	pstFrameHeader->bayerID		= bayerFormat;
	pstFrameHeader->compress	= compressMode;
	...
	for (CVI_U32 u32FrameIdx = 0; u32FrameIdx < u32TotalFrames; ++u32FrameIdx, ptYUVHeader++) {
		...
		if (ptFrameData->bTightlyMode) {
			...
			for (CVI_U32 u32PlanarIdx = 0; u32PlanarIdx < u32PlanarCount; ++u32PlanarIdx) {
				...
				u32PlaneOffset += au32PlanarSize[u32PlanarIdx];
...
static CVI_S32 CVI_ISPD2_GetMultiplyRAW_Kernel(TFrameData *ptFrameData, CVI_U32 u32TotalFrames)
{
	...
	for (CVI_U32 u32FrameIdx = 0; u32FrameIdx < u32TotalFrames; ++u32FrameIdx, ptRAWHeader++) {
		...
		for (CVI_U32 u32Idx = 0; u32Idx < ptRAWHeader->fusionFrame; ++u32Idx) {
			...
			ptRAWHeader->compress	= ptVideoFrame[u32Idx].stVFrame.enCompressMode;
...
static CVI_S32 CVI_ISPD2_GetMultiplyRAW_ByCustomVBPool(TFrameData *ptFrameData, CVI_U32 u32TotalFrames)
{
	...
	ISP_DAEMON2_DEBUG(LOG_WARNING, "GetMultiplyRAW doesn't support by using vb!");
	return CVI_FAILURE;
#endif
	TRAWHeader	*pstRAWHeader = ptFrameData->pstRAWHeader;
...
{
#ifdef V4L2_ISP_ENABLE
	ISP_DAEMON2_DEBUG(LOG_WARNING, "GetMultiplyRAW doesn't support by using vpss buffer!");
	return CVI_FAILURE;
#endif
	TRAWHeader	*pstRAWHeader = ptFrameData->pstRAWHeader;
	CVI_U32		u32FusionFrame = pstRAWHeader[0].fusionFrame;
...
static CVI_S64 CVI_ISPD2_GetIonSizeInfo(const char *path)
{
	FILE *fd = NULL;
	char cmd[256] = {0};
	...
	if (fd == NULL) {
		ISP_DAEMON2_DEBUG_EX(LOG_ERR, "fail, popen: %s\n", cmd);
		return 0;
	}
...
CVI_S32 CVI_ISPD2_CBFunc_GetRawDumpMaxFrames(TJSONRpcContentIn *ptContentIn,
	TJSONRpcContentOut *ptContentOut, JSONObject *pJsonResponse)
{
	TISPDeviceInfo		*ptDevInfo = ptContentIn->ptDeviceInfo;
	CVI_S32				ViPipe = ptDevInfo->s32ViPipe;
	CVI_U32				cropW = 0, cropH = 0;
	CVI_S32				s32Ret = CVI_SUCCESS;
	VI_DEV_ATTR_S		stDevAttr;
	VI_PIPE_ATTR_S		stPipeAttr;
	VI_CHN_ATTR_S		stChnAttr;
	CVI_U32				fullFrameSize, roiFrameSize;
	CVI_U32				frm_num;
	CVI_S64				total_mem = 0, alloc_mem = 0;
	CVI_S32				max_frame;

	if (ptContentIn->pParams) {
		GET_INT_FROM_JSON(ptContentIn->pParams, "/cropW", cropW, s32Ret);
		GET_INT_FROM_JSON(ptContentIn->pParams, "/cropH", cropH, s32Ret);
	}
	...
	if (stPipeAttr.enCompressMode == COMPRESS_MODE_NONE) {
		CVI_ISPD2_Utils_ComposeMessage(ptContentOut, JSONRPC_CODE_INVALID_REQUEST,
			"CompressMode error, must be compressed mode!");
		return CVI_FAILURE;
	}

	if ((cropW <= 0) || (cropW > stChnAttr.stSize.u32Width)
		|| (cropH <= 0) || (cropH > stChnAttr.stSize.u32Height)) {
	...
	fullFrameSize = VI_GetRawBufferSize(stChnAttr.stSize.u32Width,
	...
	roiFrameSize = VI_GetRawBufferSize(cropW,
		cropH,
		PIXEL_FORMAT_RGB_BAYER_12BPP,
		stPipeAttr.enCompressMode,
		DEFAULT_ALIGN,
	...
	total_mem = CVI_ISPD2_GetIonSizeInfo("/sys/kernel/debug/ion/cvi_vpp_heap_dump/total_mem");
	alloc_mem = CVI_ISPD2_GetIonSizeInfo("/sys/kernel/debug/ion/cvi_vpp_heap_dump/alloc_mem");

	max_frame = (total_mem - alloc_mem - fullFrameSize * frm_num * 3) / (roiFrameSize * frm_num);
	max_frame = max_frame - frm_num * 3;

	JSONObject *pDataOut = ISPD2_json_object_new_object();

	SET_INT_TO_JSON("max frames", max_frame, pDataOut);
	...
	TJSONRpcContentOut *ptContentOut, JSONObject *pJsonResponse)
{
	...
	TVPSSAdjGroupInfo	stVPSSAdjGrp;
	...
	if ((stVIVPSSMode.aenMode[pstFrameInfo->ViPipe] == VI_OFFLINE_VPSS_ONLINE)
		...
		CVI_VPSS_GetGrpProcAmp(pstFrameInfo->VpssGrp, PROC_AMP_BRIGHTNESS, &(stVPSSAdjGrp.s32Brightness));
		CVI_VPSS_GetGrpProcAmp(pstFrameInfo->VpssGrp, PROC_AMP_CONTRAST, &(stVPSSAdjGrp.s32Contrast));
		CVI_VPSS_GetGrpProcAmp(pstFrameInfo->VpssGrp, PROC_AMP_SATURATION, &(stVPSSAdjGrp.s32Saturation));
		CVI_VPSS_GetGrpProcAmp(pstFrameInfo->VpssGrp, PROC_AMP_HUE, &(stVPSSAdjGrp.s32Hue));
	...
	if (bSetVPSSDefault) {
		CVI_VPSS_SetGrpProcAmp(pstFrameInfo->VpssGrp, PROC_AMP_BRIGHTNESS, stVPSSAdjGrp.s32Brightness);
		CVI_VPSS_SetGrpProcAmp(pstFrameInfo->VpssGrp, PROC_AMP_CONTRAST, stVPSSAdjGrp.s32Contrast);
		CVI_VPSS_SetGrpProcAmp(pstFrameInfo->VpssGrp, PROC_AMP_SATURATION, stVPSSAdjGrp.s32Saturation);
		CVI_VPSS_SetGrpProcAmp(pstFrameInfo->VpssGrp, PROC_AMP_HUE, stVPSSAdjGrp.s32Hue);
	}
	...
	TJSONRpcContentOut *ptContentOut, JSONObject *pJsonResponse)
{
	...
	if (ptContentIn->pParams) {
		...
		GET_INT_FROM_JSON(ptContentIn->pParams, "/imageSrcIndex", s32ImageSrcVipe, s32ImageSrcVipeState);
	...
	if (eImageSrc == IMAGE_FROM_VI) {
		...
		if ((stVIVPSSMode.aenMode[ViPipe] == VI_OFFLINE_VPSS_ONLINE)
			...
			// backup and apply default VPSS Adjustment
			CVI_VPSS_GetGrpProcAmp(VpssGrp, PROC_AMP_BRIGHTNESS, &(stVPSSAdjGrp.s32Brightness));
			CVI_VPSS_GetGrpProcAmp(VpssGrp, PROC_AMP_CONTRAST, &(stVPSSAdjGrp.s32Contrast));
			CVI_VPSS_GetGrpProcAmp(VpssGrp, PROC_AMP_SATURATION, &(stVPSSAdjGrp.s32Saturation));
			CVI_VPSS_GetGrpProcAmp(VpssGrp, PROC_AMP_HUE, &(stVPSSAdjGrp.s32Hue));
	...
	// restore VPSS adjustment
	if (bSetVPSSDefault) {
		CVI_VPSS_SetGrpProcAmp(VpssGrp, PROC_AMP_BRIGHTNESS, stVPSSAdjGrp.s32Brightness);
		CVI_VPSS_SetGrpProcAmp(VpssGrp, PROC_AMP_CONTRAST, stVPSSAdjGrp.s32Contrast);
		CVI_VPSS_SetGrpProcAmp(VpssGrp, PROC_AMP_SATURATION, stVPSSAdjGrp.s32Saturation);
		CVI_VPSS_SetGrpProcAmp(VpssGrp, PROC_AMP_HUE, stVPSSAdjGrp.s32Hue);
	}
...
static CVI_S32 CVI_ISPD2_UpdateRegDump(TFrameData *ptFrameData)
{
	...
	ptFrameData->pu8RegDump = (CVI_U8 *)calloc(lFileSize + 1, sizeof(CVI_U8));

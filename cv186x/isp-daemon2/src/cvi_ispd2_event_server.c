	...
	TISPDaemon2ConnectInfo *ptConnectObj)
{
	...
	if (!bBinaryMode) {
		...
		eDataState = CVI_ISPD2_Message_CommandChecker(&(ptConnectObj->stPacketInfo),
			pszBuf, u32BufOffset, u32ContentTailPos);
...
// -----------------------------------------------------------------------------
static void CVI_ISPD2_ES_CB_SocketRead(cvi_uv_stream_t *pUVClientHandle, ssize_t lReadLength, cvi_uv_buf_t *pBuf)
{
...
static void CVI_ISPD2_ES_CB_NewConnection(cvi_uv_stream_t *pUVServerHandle, int iStatus)
{
	...
	if (cvi_uv_accept(pUVServer, (cvi_uv_stream_t *)&(pUVClientEx->UVTcp)) == 0) {
		...
		ptConnectObj->stPacketInfo.u32SquareBracketNum = 0;
		ptConnectObj->stPacketInfo.u32BracketNum = 0;
